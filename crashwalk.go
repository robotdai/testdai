...
func Summarize(c crash.Crash) string {
	...
	fmt.Fprintf(&buf, "当前crash判定信息展示\n")
	fmt.Fprintf(&buf, "crash文件名: %s\n", c.OrigFilename)
	//fmt.Fprintf(&buf, "SHA1: %s\n", hex.EncodeToString(c.SHA1))
	fmt.Fprintf(&buf, "可利用性判定结果: %s\n", c.Classification)
	fmt.Fprintf(&buf, "当前crash的Hash值: %s\n", c.Hash)
	//fmt.Fprintf(&buf, "Command: %s\n", strings.Join(c.Command, " "))
	fmt.Fprintf(&buf, "崩溃地址:\n")
	ff := c.FaultingFrame
	fmt.Fprintf(&buf, "   %s @ 0x%.16x: in %s\n", ff.Symbol, ff.Address, ff.Module)
	fmt.Fprintf(&buf, "崩溃点及寄存器:\n")
	...
	fmt.Fprintf(&buf, "\n")
	fmt.Fprintf(&buf, "崩溃属性归类:\n")
	for _, l := range c.Extra {
		fmt.Fprintf(&buf, "   %s\n", l)
	}
	fmt.Fprintf(&buf, "判定结束\n")
	fmt.Fprintf(&buf, "\n")
	fmt.Fprintf(&buf, "\n")
	return buf.String()
}
